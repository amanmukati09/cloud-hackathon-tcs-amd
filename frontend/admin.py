...
def fetch_gamification_stats(token):
    """Fetch user's gamification stats."""
    if not token:
        return "", pd.DataFrame(), pd.DataFrame()
    try:
        res = requests.get(
            f"{BACKEND_URL}/gamification/stats",
            headers={"Authorization": f"Bearer {token}"},
            timeout=10
        )
        if res.status_code == 200:
            ...
            progress = level.get("progress_percent", 0)
            
            points_html = f"""
            <div style="text-align:center;padding:12px;">
                <h2 style="color:#f59e0b;font-size:2rem;margin:0;">⭐ {points:,} Points</h2>
                <p style="color:#94a3b8;margin:4px 0;">Level {level.get('level', 1)}</p>
                <div style="background:rgba(0,0,0,0.3);border-radius:10px;height:8px;margin:8px 0;">
                    <div style="background:linear-gradient(90deg,#f59e0b,#10b981);height:8px;border-radius:10px;width:{progress}%;"></div>
                </div>
                <p style="color:#64748b;font-size:0.75em;">{level.get('points_to_next_level', 100)} pts to Level {level.get('level', 1) + 1}</p>
            </div>
            """
            
            # Badges
            badges = data.get("badges", [])
            badges_html = '<div style="display:flex;flex-wrap:wrap;gap:6px;justify-content:center;">'
            for b in badges:
                badges_html += f'<span style="background:rgba(56,189,248,0.1);border:1px solid rgba(56,189,248,0.2);padding:4px 10px;border-radius:20px;font-size:0.8em;">{b["icon"]} {b["name"]}</span>'
            badges_html += '</div>'
            
            return points_html, badges_html, pd.DataFrame()
    except:
        pass
    return "", "", pd.DataFrame()
...
def fetch_leaderboard(token):
    ...
    try:
        res = requests.get(
            f"{BACKEND_URL}/gamification/leaderboard",
            headers={"Authorization": f"Bearer {token}"},
            timeout=10
        )
        if res.status_code == 200:
